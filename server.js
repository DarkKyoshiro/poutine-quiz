...
var quizID = "poutinequizS03E03-2024SeptemberXX";
mongoose
  .connect(
...
io.on("connection", (socket) => {
  ...
  socket.on("show-question", () => {
    questions[questionID - 1].showQuestion = questions[questionID - 1]
      .showQuestion
      ? false
      : true;
  ...
  socket.on("show-answer", () => {
    questions[questionID - 1].showAnswer = questions[questionID - 1].showAnswer
      ? false
      : true;
    io.emit("send-questions", questions, menuTeamGroup);
  });
    ...
    if (bonusesWrongAnswers) {
      ...
      for (const key in teams) {
        ...
        if (teams[key].score === firstSmallestScore) {
          ...
          Math.round(
            (100 * (numberTeamsTier1 + numberTeamsTier2)) / numberAnswers
          ) <
