...
export class AdminGameComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.socket.on('get-answers-team', (data: any) => {
      ...
      if (
        this.getNumberAnswers() >= this.teams.length / this.divider &&
        this.getNumberAnswers() - 1 < this.teams.length / 3 &&
        (this.questions[this.questionID - 1].type === 'Addition' ||
          this.questions[this.questionID - 1].type === 'SelOuPoivre')
      ) {
        this._snackBar.open(
          'Au moins un tier des équipes ont répondus. Vous pouvez arrêter ici.',
          'OK',
          {
            duration: this.durationInSeconds * 1000,
          }
  ...
  getRanks(teamName: string): number {
    ...
    sortedArray = timestampArray.slice().sort((a, b) => {
      return a - b;
    });
    rankings = timestampArray.map((v) => {
  ...
  isMenuActive(teamName: string): boolean {
    if (!this.questions[this.questionID - 1].group) {
      ...
      this.menuDistribution.forEach((menu) => {
        if (menu.menuNb === menuNb && menu.round === round) {
  ...
  isCorrected(): boolean {
    ...
    this.answers.forEach((answer) => {
      if (answer.correct >= 0 && answer.questionID === this.questionID) {
        correctedAnswers++;
      }
    });
  ...
  getTimeString(): { result: string; correct: number; bonus: number }[] {
    ...
    answers.forEach((answer) => {
      ...
      seconds =
        Math.trunc(
          ((answer.timestamp - fastestTime) / 1000 - minutes * 60) * 100
        ) / 100;

      minutes === 0
        ? (tempString = answer.teamName + ': ' + seconds + 'sec')
        : (tempString =
            answer.teamName + ': ' + minutes + 'mins ' + seconds + 'sec');

      timeStrings.push({
        result: tempString,
        correct: answer.correct,
        bonus: answer.bonusSpeed,
      });
    });
  ...
  getBonusWrongAnswersString(): { team: string; points: number }[] {
    ...
    answers = answers.sort((a, b) =>
      b.bonusWrongAnswers < a.bonusWrongAnswers ? 1 : -1
    );

    answers.forEach((answer) => {
      bonusStrings.push({
        team: answer.teamName,
        points: answer.bonusWrongAnswers,
      });
    });
  ...
  getNumberAnswers(): number {
    ...
    numberAnswers = this.answers.filter(answer => answer.questionID === this.questionID && answer.answer !== '').length
