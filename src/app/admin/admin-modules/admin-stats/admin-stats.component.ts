...
export class AdminStatsComponent implements OnInit {
    ...
    mostCorrectAnswers(): string {
        ...
        this.answers.forEach((answer) => {
            if (!teams.has(answer.teamName)) {
                teams.set(answer.teamName, 0)
            }
            if (answer.correct === 1) {
                teams.set(answer.teamName, teams.get(answer.teamName)! + 1)
            }
        })
    ...
    fastestTeamAdded(): string {
        ...
        this.answers.forEach((answer) => {
            if (!teamsAdded.has(answer.teamName)) {
                teamsAdded.set(answer.teamName, 0)
            }
            if (answer.correct !== -1) {
                teamsAdded.set(
                    answer.teamName,
    ...
    remontadaTable(): {
        ...
        this.answers.forEach((answer) => {
            if (!teamsRd1.has(answer.teamName)) {
                teamsRd1.set(answer.teamName, 0)
            }
            if (!teamsRd2.has(answer.teamName)) {
                teamsRd2.set(answer.teamName, 0)
            }
            if (this.questions[answer.questionID - 1]?.round === 1) {
                teamsRd1.set(
                    answer.teamName,
        ...
        if (teamsRd1Ranked.size > 0 && teamsRd2Ranked.size > 0) {
            for (let [key, value] of teamsRd1Ranked) {
                finalTeams.push({
                    name: key,
                    rd1Rank: value,
                    rd1Score: teamsRd1.get(key),
                    rd2Rank: teamsRd2Ranked.get(key),
                    rd2Score: teamsRd2.get(key),
                })
            }

            finalTeams = finalTeams.slice().sort((a, b) => {
                return (
                    (b.rd2Score - b.rd1Score) / b.rd1Score - (a.rd2Score - a.rd1Score) / a.rd1Score
                )
            })
            finalTeams = finalTeams.slice().sort((a, b) => {
                return a.rd2Rank - a.rd1Rank - (b.rd2Rank - b.rd1Rank)
            })
